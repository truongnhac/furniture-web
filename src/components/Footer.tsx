import { Link } from 'react-router-dom';
function FacebookIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}
function ZaloIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      <path d="M12.49 10.2722v-.4496h1.3467v6.3218h-.7704a.576.576 0 01-.5763-.5729l-.0006.0005a3.273 3.273 0 01-1.9372.6321c-1.8138 0-3.2844-1.4697-3.2844-3.2823 0-1.8125 1.4706-3.2822 3.2844-3.2822a3.273 3.273 0 011.9372.6321l.0006.0005zM6.9188 7.7896v.205c0 .3823-.051.6944-.2995 1.0605l-.03.0343c-.0542.0615-.1815.206-.2421.2843L2.024 14.8h4.8948v.7682a.5764.5764 0 01-.5767.5761H0v-.3622c0-.4436.1102-.6414.2495-.8476L4.8582 9.23H.1922V7.7896h6.7266zm8.5513 8.3548a.4805.4805 0 01-.4803-.4798v-7.875h1.4416v8.3548H15.47zM20.6934 9.6C22.52 9.6 24 11.0807 24 12.9044c0 1.8252-1.4801 3.306-3.3066 3.306-1.8264 0-3.3066-1.4808-3.3066-3.306 0-1.8237 1.4802-3.3044 3.3066-3.3044zm-10.1412 5.253c1.0675 0 1.9324-.8645 1.9324-1.9312 0-1.065-.865-1.9295-1.9324-1.9295s-1.9324.8644-1.9324 1.9295c0 1.0667.865 1.9312 1.9324 1.9312zm10.1412-.0033c1.0737 0 1.945-.8707 1.945-1.9453 0-1.073-.8713-1.9436-1.945-1.9436-1.0753 0-1.945.8706-1.945 1.9436 0 1.0746.8697 1.9453 1.945 1.9453z" />
    </svg>
  );
}
export default function Footer() {
  return (
    <footer className="bg-stone-900 text-stone-400 mt-24">
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-3 gap-12">
        {/* Brand */}
        <div>
           <img
  src="/LOGO trang.png"
  alt="Orange Moon Logo"
   className="w-30 h-30 object-contain bg-transparent"
/>
          <p className="text-xs tracking-[0.3em] uppercase text-stone-500 mb-4">ORANGE MOON DECOR</p>
          <p className="text-sm leading-relaxed">
            Architecture · Interior · Furniture<br />
            Based in Daklak, Vietnam.
          </p>
        </div>

        {/* Nav */}
        <div>
          <p className="text-xs tracking-widest uppercase text-stone-500 mb-4">Điều Hướng</p>
          <ul className="space-y-2 text-sm">
            {[
              { to: '/', label: 'Trang Chủ' },
              { to: '/projects', label: 'Dự Án' },
              { to: '/products', label: 'Sản Phẩm' },
              { to: '/about', label: 'Giới Thiệu' },
              { to: '/contact', label: 'Liên Hệ' },
            ].map(({ to, label }) => (
              <li key={to}>
                <Link to={to} className="hover:text-white transition-colors duration-200">
                  {label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Contact */}
        <div>
          <p className="text-xs tracking-widest uppercase text-stone-500 mb-4">Liên Hệ</p>
          <ul className="space-y-2 text-sm">
            <li>Thôn 3, xã Ea Ktur, huyện Cư Kuin, tỉnh Đăk Lăk</li>
            <li>
              <a href="tel:+84357996872" className="hover:text-white transition-colors">
                0903 711 160
              </a>
            </li>
            <li>
              <a href="mailto:quynhnguyen261117@gmail.com" className="hover:text-white transition-colors">
                quynhnguyen261117@gmail.com
              </a>
            </li>
            <li>
              <div className="flex items-center gap-2 text-stone-400">
                <a href="https://www.facebook.com/profile.php?id=100044457139917" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors p-1">
                  <FacebookIcon />
                </a>
                <a href="https://raw.githubusercontent.com/truongnhac/furniture-web/master/image/Quynh-Nguyen-Zalo.png" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors p-1">
                  <ZaloIcon />
                </a>
              </div>
            </li>
          </ul>
        </div>
      </div>

      <div className="border-t border-stone-800 text-center py-4 text-xs text-stone-600">
        © {new Date().getFullYear()} ORANGE MOON DECOR
      </div>
    </footer>
  );
}
